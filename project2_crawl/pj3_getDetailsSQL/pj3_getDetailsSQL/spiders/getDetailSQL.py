...
from ..items import Pj3GetdetailssqlItem
class GetDetail(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        yield items
        GetDetail.linknumber += 1
        next_link= GetDetail.urls[GetDetail.linknumber]
        if next_link is not None:
            yield response.follow(url=next_link, callback=self.parse)#, dont_filter=True